...
static int init_restore (supercrack_ctx_t *supercrack_ctx)
{
...
static int read_restore (supercrack_ctx_t *supercrack_ctx)
{
  restore_ctx_t   *restore_ctx   = supercrack_ctx->restore_ctx;
  ...
  {
    event_log_error (supercrack_ctx, "Restore file '%s': %s", eff_restore_file, strerror (errno));
  ...
  {
    event_log_error (supercrack_ctx, "Cannot read %s", eff_restore_file);
  ...
  if (rd->argc < 1)
  {
    event_log_error (supercrack_ctx, "Unusually low number of arguments (argc) within restore file %s", eff_restore_file);
  ...
  {
    event_log_error (supercrack_ctx, "Unusually high number of arguments (argc) within restore file %s", eff_restore_file);
  ...
  {
    event_log_error (supercrack_ctx, "%s: %s", rd->cwd, strerror (errno));
  ...
  {
    event_log_warning (supercrack_ctx, "Changing current working directory to '%s'", rd->cwd);
    event_log_warning (supercrack_ctx, NULL);

    if (chdir (rd->cwd))
    {
      event_log_error (supercrack_ctx, "Directory '%s' needed to restore the session was not found.", rd->cwd);

      event_log_warning (supercrack_ctx, "Either create the directory, or update the directory within the .restore file.");
      event_log_warning (supercrack_ctx, "Restore files can be analyzed and modified with analyze_hc_restore.pl:");
      event_log_warning (supercrack_ctx, "    https://github.com/philsmd/analyze_hc_restore");
      event_log_warning (supercrack_ctx, "Directory must contain all files and folders from the original command line.");
      event_log_warning (supercrack_ctx, NULL);
    ...
    const int rc_folder_config_init = folder_config_init (supercrack_ctx, install_folder, shared_folder);
...
static int write_restore (supercrack_ctx_t *supercrack_ctx)
{
  const mask_ctx_t     *mask_ctx     = supercrack_ctx->mask_ctx;
  const restore_ctx_t  *restore_ctx  = supercrack_ctx->restore_ctx;
  ...
  {
    event_log_error (supercrack_ctx, "%s: %s", new_restore_file, strerror (errno));
  ...
  {
    event_log_error (supercrack_ctx, "setvbuf file '%s': %s", new_restore_file, strerror (errno));
...
int cycle_restore (supercrack_ctx_t *supercrack_ctx)
{
  ...
  if (write_restore (supercrack_ctx) == -1) return -1;

  if (hc_path_exist (eff_restore_file) == true)
  {
    if (unlink (eff_restore_file) == -1)
    {
      event_log_warning (supercrack_ctx, "Unlink file '%s': %s", eff_restore_file, strerror (errno));
    }
  }

  if (rename (new_restore_file, eff_restore_file) == -1)
  {
    event_log_warning (supercrack_ctx, "Rename file '%s' to '%s': %s", new_restore_file, eff_restore_file, strerror (errno));
  }
...
int restore_ctx_init (supercrack_ctx_t *supercrack_ctx, int argc, char **argv)
{
  ...
  if (init_restore (supercrack_ctx) == -1) return -1;
  ...
  {
    if (read_restore (supercrack_ctx) == -1) return -1;

    restore_data_t *rd = restore_ctx->rd;

    if (rd->version < RESTORE_VERSION_MIN)
    {
      event_log_error (supercrack_ctx, "Incompatible restore-file version.");

      return -1;
    }

    user_options_init (supercrack_ctx);

    if (user_options_getopt (supercrack_ctx, rd->argc, rd->argv) == -1) return -1;
