...
_supercrack_backend_devices ()
{
  local cur_selection="${1}"
  supercrack_device_list=""

  local executable="${SUPERCRACK_ROOT}"/supercrack

  if [ ! -x "${executable}" ]; then
    executable="${SUPERCRACK_ROOT}"/supercrack.bin
  fi

  if [ ! -x "${executable}" ]; then
    local which_supercrack=$(which supercrack 2>/dev/null)

    if [ -n "${which_supercrack}" ]; then
      executable="${which_supercrack}"
    fi
  fi
  ...
  local supercrack_I=$("${executable}" -I 2>/dev/null | grep "Backend Device ID #[0-9]\+" | sed 's/^ *Backend Device ID #//')

  if [ -z "${supercrack_I}" ]; then
    return
  fi

  local aliases=$(echo "${supercrack_I}" | grep "(Alias: #[0-9]\+)" | sed 's/ *(Alias: #\([0-9]\+\))/ \1/')
  ...
  local device_list=$(echo "${supercrack_I}" | grep -o "^[0-9]*" | sort -n -u)
  ...
  local supercrack_backend_list=""

  if [ -n "${cur_selection}" ]; then
    supercrack_backend_list="${cur_selection}"
  fi

  for device in ${appended_device_list}; do
    if [ -z "${supercrack_backend_list}" ]; then
      if [ -z "${cur_selection}" ]; then
        supercrack_backend_list="${device}"
      else
        supercrack_backend_list="${cur_selection},${device}"
      fi
    else
      if [ -z "${cur_selection}" ]; then
        supercrack_backend_list=$(echo -e "${supercrack_backend_list}\n${device}")
      else
  ...
  for device_str in ${supercrack_backend_list}; do
    ...
    if [ -z "${supercrack_device_list}" ]; then
      supercrack_device_list="${device_str}"
    else
      supercrack_device_list=$(echo -e "${supercrack_device_list}\n${device_str}")
    fi
  done
}

_supercrack_cpu_devices ()
{
  local cur_selection="${1}"
  ...
  if [ -n "${cur_selection}" ]; then
    supercrack_device_list="${cur_selection}"
  fi

  for device in ${appended_device_list}; do
    if [ -z "${supercrack_device_list}" ]; then
      if [ -z "${cur_selection}" ]; then
        supercrack_device_list="${device}"
      else
        supercrack_device_list="${cur_selection},${device}"
      fi
    else
      if [ -z "${cur_selection}" ]; then
        supercrack_device_list=$(echo -e "${supercrack_device_list}\n${device}")
...
_supercrack_files_replace_home ()
{
  local cur_select="${1}"
  local  cur_files="${2}"

  supercrack_select="${cur_select}"
  supercrack_file_list="${cur_files}"

  if echo ${cur_select} | grep -q "^~/"; then
    home_dir="${HOME}"

    if [ -n "${home_dir}" ]; then
      supercrack_file_list=$(echo -n "${cur_files}" | sed "s!^${home_dir}!~\\\\!")
      supercrack_select=$(echo -n "${cur_select}" | sed "s!^~/!~\\\\/!")
    fi
  fi
}

_supercrack_recursive_file_search ()
{
  local  allow_dir="${1}"
  local is_include="${2}"
  local  file_list="${3}"
    ...
    if [ -d "${dir_loop}" ]; then
      ...
      for loop_cnt in $(seq 1 35); do # maximum number of recursive (subdir) tests
        ...
        if [ "${allow_dir}" -eq 1 ]; then
          if [ -n "${supercrack_file_list}" ]; then
            supercrack_file_list="${supercrack_file_list} "
          fi

          supercrack_file_list="${supercrack_file_list}${subdir}"
        fi
        ...
        for subdir_file in "${subdir_files}"; do
          if [ "${allow_dir}" -eq 1 ]; then
            ...
            supercrack_file_list="${supercrack_file_list}${subdir_file}"
          else
      ...
      supercrack_file_list="${supercrack_file_list}${dir_loop}"
    fi
  done
}
...
_supercrack_include ()
{
  ...
  _supercrack_recursive_file_search "${allow_dir}" 1 "${file_list}" "${cur_filter}"
  ...
  if [ "${allow_dir}" -eq 1 ]; then
    if [ -d "${cur_select}" ]; then
      ...
      supercrack_file_list="${supercrack_file_list}${cur_select}"
    fi
  fi

  # handle special case for $HOME directory (~/)

  _supercrack_files_replace_home "${cur_select}" "${supercrack_file_list}"

  # (supercrack_select and supercrack_file_list are modified and "returned")
}

_supercrack_files_include ()
{
  _supercrack_include 0 "${1}" "${2}"
}

_supercrack_files_folders_include ()
{
  _supercrack_include 1 "${1}" "${2}"
}
...
_supercrack_exclude ()
{
  ...
  _supercrack_recursive_file_search "${allow_dir}" 0 "${file_list}" "${cur_filter}"
...
_supercrack_files_exclude ()
{
  _supercrack_exclude 0 "${1}" "${2}"
}

_supercrack_files_folders_exclude ()
{
  _supercrack_exclude 1 "${1}" "${2}"
}

_supercrack_contains ()
{
  local haystack=${1}
  local needle="${2}"
...
_supercrack ()
{
  ...
  case "${prev}" in
    ...
    -o|--outfile|-r|--rules-file|--debug-file|--potfile-path| --restore-file-path)
      _supercrack_files_exclude "${cur}" "${HIDDEN_FILES_AGGRESSIVE}"
      COMPREPLY=($(compgen -W "${supercrack_file_list}" -- ${supercrack_select})) # or $(compgen -f -X '*.+('${HIDDEN_FILES_AGGRESSIVE}')' -- ${cur})
      return 0
      ;;

    --markov-hcstat2)
      _supercrack_files_include "${cur}" '.*\.hcstat2$'
      COMPREPLY=($(compgen -W "${supercrack_file_list}" -- ${supercrack_select}))
      return 0
      ;;

     -d|--backend-devices)
      _supercrack_backend_devices ${cur}

      COMPREPLY=($(compgen -W "${supercrack_device_list}" -- ${cur}))
      return 0
      ;;
    ...
    --keyboard-layout-mapping)
      _supercrack_files_include "${cur}" '.*\.hckmap$'
      COMPREPLY=($(compgen -W "${supercrack_file_list}" -- ${supercrack_select}))
      return 0
      ;;
    ...
    -1|-2|-3|-4|--custom-charset1|--custom-charset2|--custom-charset3|--custom-charset4)
      ...
      _supercrack_files_exclude "${cur}" "${HIDDEN_FILES}"

      mask="${mask} ${supercrack_file_list}"

      COMPREPLY=($(compgen -W "${mask}" -- ${supercrack_select}))
      return 0
      ;;
    ...
    --truecrypt-keyfiles|--veracrypt-keyfiles)
      ...
      _supercrack_files_exclude "${cur_part2}" "${HIDDEN_FILES_AGGRESSIVE}"


      # generate lines with the file name and a duplicate of it with a comma at the end

      supercrack_file_list=$(echo "${supercrack_file_list}"  | \
                          sed  "s/^/${cur_part1}/"     | \
                          sed  "s/^/${cur_part0}/"     | \
                          sed  's/^\(.*\)$/\1\n\1,\n/' | \
                          sed  's/,\+$/,/g'            | \
      ...
      COMPREPLY=($(compgen -W "${supercrack_file_list}" -- ${cur_sel}))
      return 0
  esac
  ...
  case "$cur" in
    ...
    -d*)
      _supercrack_backend_devices $(echo ${cur} | sed 's/^-d//')

      local supercrack_devices_permutation="$(echo -e "\n${supercrack_device_list}" | sed 's/^/-d/')"
      COMPREPLY=($(compgen -W "${supercrack_devices_permutation}" -- ${cur}))
      return 0
      ;;
  esac
  ...
  local h=1
  local no_opts=0
  local attack_mode=0 # also default of supercrack
  ...
  while [ ${h} -le ${COMP_CWORD} ]; do
    ...
    if _supercrack_contains "${OPTIONS}" "${COMP_WORDS[h]}"; then

      h=$((h + 2))

    else

      if ! _supercrack_contains "${LONG_OPTS}${SHORT_OPTS}" "${COMP_WORDS[h]}"; then
    ...
    *)
      case "${attack_mode}" in
        ...
        0|9)
          ...
          _supercrack_files_folders_exclude "${cur}" "${HIDDEN_FILES_AGGRESSIVE}"
          COMPREPLY=($(compgen -W "${supercrack_file_list}" -- ${supercrack_select}))
          return 0
          ;;
        ...
        6)
          if [ "${no_opts}" -eq 2 ]; then
            ...
            _supercrack_files_folders_exclude "${cur}" "${HIDDEN_FILES}"
...
complete -F _supercrack "${SUPERCRACK_ROOT}"/supercrack.bin  "${SUPERCRACK_ROOT}"/supercrack supercrack
